test_that("clean(verbose = FALSE) removes all output files generated by scripts", {
  temp_dir <- tempdir()
  old_wd <- getwd()
  setwd(temp_dir)
...
'
  ...
  result <- capture.output({result_value <- clean(verbose = FALSE)}); result <- result_value
...
test_that("clean(verbose = FALSE) returns empty vector when no output files exist", {
  temp_dir <- tempdir()
  old_wd <- getwd()
  setwd(temp_dir)
...
test_that("clean(verbose = FALSE) only removes files that exist", {
  temp_dir <- tempdir()
  old_wd <- getwd()
  setwd(temp_dir)
...
test_that("clean(verbose = FALSE) handles scripts with no outputs", {
  temp_dir <- tempdir()
  old_wd <- getwd()
  setwd(temp_dir)
...
test_that("clean(verbose = FALSE) removes files that are both inputs and outputs", {
  temp_dir <- tempdir()
  old_wd <- getwd()
  setwd(temp_dir)
